...
- Funcionalidades complejas (Checkout, Cart, Auth, Shop, Orders, Navigation Chips, Login, Testimonials, Hero) deben tener sus propios archivos y cargarse condicionalmente.
...
│   ├── checkout.php           # ✅ Checkout Híbrido Optimizado + Login Gate
...
│   │                          #   §1-3: Tarjetas producto/categoría (bestsellers + popcat)
│   │                          #   §4+: Hero Promos (.mu-hero-promo-*) — slider grid-stack, dots, badge
│   ├── components/
...
inc/addon-etiquetas.php | Addon Etiquetas v3.0: builder, config, render UI, enqueue JS+config.
...
js/global-ui.js | ✅ Global — initCarousels() cubre todos los .mu-carousel-wrapper
js/header.js | Global
js/footer.js | Global
...
js/testimonials.js | has_shortcode('mu_testimonios_section') — IIFE, Fisher-Yates shuffle, render dinámico, botón satélite. Datos PHP→JS vía muTestimonials.reviews.
...
Slider Hero promos dinámicas | ui.php | home.css | hero.js
...
- Shuffle arrays: SIEMPRE usar Fisher-Yates. NUNCA sort(() => 0.5 - Math.random()).
- Carrusel: NUNCA duplicar initCarousels(). Todo .mu-carousel-wrapper es automático.
- Hero slider: Controla SOLO elementos .mu-hero-promo-slide. Dots identificados por data-hero-dot (sin onclick inline). Swipe con {passive:true}.
...
- [ACCIÓN REQUERIDA] Desactivar snippet con [mu_bestsellers_section] / [mu_popcat_section] en Code Snippets.
- [ACCIÓN REQUERIDA] Desactivar snippet con [mu_hero_section] en Code Snippets al mergear este PR.
