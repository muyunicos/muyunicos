...
if ( ! function_exists( 'mu_navchips_admin_bar_button' ) ) {
    function mu_navchips_admin_bar_button( $wp_admin_bar ) {
        ...
        $wp_admin_bar->add_node(
            [
                'id'    => 'mu-navchips-rebuild-index',
                'title' => '🔄 Regenerar Índice NavChips',
                'href'  => wp_nonce_url( admin_url( 'admin-post.php?action=mu_navchips_rebuild_index' ), 'mu_navchips_rebuild' ),
            ]
        );
    }
    add_action( 'admin_bar_menu', 'mu_navchips_admin_bar_button', 100 );
}

if ( ! function_exists( 'mu_navchips_handle_manual_rebuild' ) ) {
    function mu_navchips_handle_manual_rebuild() {
        check_admin_referer( 'mu_navchips_rebuild' );
        ...
        wp_redirect( add_query_arg( 'mu_navchips_rebuilt', '1', wp_get_referer() ) );
        exit;
    }
    add_action( 'admin_post_mu_navchips_rebuild_index', 'mu_navchips_handle_manual_rebuild' );
}

if ( ! function_exists( 'mu_navchips_admin_notice' ) ) {
    function mu_navchips_admin_notice() {
        if ( ! isset( $_GET['mu_navchips_rebuilt'] ) ) {
            return;
        }
        ...
        if ( $meta ) {
            printf(
                'Productos: %d | Tamaño: %s KB | Tiempo: %s ms',
                (int) $meta['total_products'],
                esc_html( $size_kb ),
                esc_html( $meta['generation_time_ms'] )
            );
        }
        echo '</p></div>';
    }
    add_action( 'admin_notices', 'mu_navchips_admin_notice' );
}
